...
- noise：无意义的碎片、品牌 logo、背景装饰文字、明显的错识别、单独的数字或符号。地图上密密麻麻的地名与编号、报纸版面里的零碎文字、背景里成片的标注也算 noise——它们不是给观众读的，逐条音译出来只会糊住画面。
sign 再给一个重要度 importance：3 = 不看就不懂剧情（短信、书信、便签、文件内容、标题卡、关键告示）；2 = 有帮助（店名、招牌、路牌、标签、价签、书名）；1 = 可有可无（界面按钮、菜单项、装饰性文字、背景书架杂志封面、商品包装上的字、海报或书封上罗列的人名、地图上的地名标注）。
