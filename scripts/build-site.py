...
T['zh'] = dict(
  ...
  req_notes=['「运行占用内存」是模型加载后实际占用；识别与翻译先后进行，不会同时占用两份。',
             'Apple Silicon 用 Metal 加速；Windows 版语音识别在 CPU 上运算（速度明显慢于同档 Mac），翻译在装有 Vulkan 驱动的显卡上运行、没有则回落 CPU；不支持 Intel Mac。',
...
T['en'] = dict(
  ...
  req_notes=['"RAM in use" is what the loaded model actually occupies; recognition and translation run one after the other, never both at once.',
             'Apple Silicon uses Metal acceleration. On Windows, speech recognition runs on the CPU (noticeably slower than a comparable Mac) while translation runs on any GPU with a Vulkan driver, falling back to the CPU otherwise. Intel Macs are not supported.',
...
T['ja'] = dict(
  ...
  req_notes=['「使用メモリ」はモデル読み込み後の実占有量。認識と翻訳は順に実行され、同時には占有しません。',
             'Apple Silicon は Metal で高速化。Windows 版は音声認識が CPU 処理（同クラスの Mac よりかなり遅い）、翻訳は Vulkan ドライバのある GPU で実行され、ない場合は CPU にフォールバックします。Intel Mac は非対応です。',
...
T['ko'] = dict(
  ...
  req_notes=['"사용 메모리"는 모델을 불러온 뒤 실제 점유량. 인식과 번역은 차례로 실행되어 동시에 점유하지 않습니다.',
             'Apple Silicon은 Metal 가속. Windows 버전은 음성 인식이 CPU에서 실행되고(같은 급 Mac보다 눈에 띄게 느림), 번역은 Vulkan 드라이버가 있는 GPU에서 실행되며 없으면 CPU로 대체됩니다. Intel Mac은 지원하지 않습니다.',
...
T['fr'] = dict(
  ...
  req_notes=['« RAM utilisée » est l\'occupation réelle du modèle chargé ; reconnaissance et traduction s\'enchaînent, jamais en même temps.',
             'Apple Silicon utilise l\'accélération Metal. Sous Windows, la reconnaissance vocale tourne sur le CPU (nettement plus lente qu\'un Mac équivalent) et la traduction sur tout GPU doté d\'un pilote Vulkan, avec repli sur le CPU sinon. Les Mac Intel ne sont pas pris en charge.',
...
T['de'] = dict(
  ...
  req_notes=['„RAM im Betrieb“ ist die tatsächliche Belegung des geladenen Modells; Erkennung und Übersetzung laufen nacheinander, nie gleichzeitig.',
             'Apple Silicon nutzt Metal-Beschleunigung. Unter Windows läuft die Spracherkennung auf der CPU (deutlich langsamer als ein vergleichbarer Mac), die Übersetzung auf jeder GPU mit Vulkan-Treiber, sonst ebenfalls auf der CPU. Intel-Macs werden nicht unterstützt.',
...
T['ru'] = dict(
  ...
  req_notes=['«Память при работе» — фактическое потребление загруженной модели; распознавание и перевод идут по очереди, а не одновременно.',
             'Apple Silicon использует ускорение Metal. В Windows распознавание речи работает на CPU (заметно медленнее сопоставимого Mac), а перевод — на любой видеокарте с драйвером Vulkan, иначе тоже на CPU. Intel Mac не поддерживаются.',
...
T['id'] = dict(
  ...
  req_notes=['"RAM saat dipakai" adalah pemakaian nyata setelah model dimuat; pengenalan dan terjemahan berjalan bergantian, tidak bersamaan.',
             'Apple Silicon memakai akselerasi Metal. Di Windows, pengenalan suara berjalan di CPU (jauh lebih lambat dari Mac sekelas), sedangkan terjemahan berjalan di GPU apa pun yang punya driver Vulkan dan kembali ke CPU bila tidak ada. Mac Intel tidak didukung.',
...
T['ms'] = dict(
  ...
  req_notes=['"RAM semasa digunakan" ialah penggunaan sebenar selepas model dimuatkan; pengecaman dan terjemahan berjalan bergilir, bukan serentak.',
             'Apple Silicon menggunakan pecutan Metal. Di Windows, pengecaman suara berjalan pada CPU (jauh lebih perlahan daripada Mac setara), manakala terjemahan berjalan pada mana-mana GPU yang mempunyai pemacu Vulkan dan kembali ke CPU jika tiada. Mac Intel tidak disokong.',
...
T['vi'] = dict(
  ...
  req_notes=['"RAM khi chạy" là mức chiếm dụng thực tế sau khi nạp mô hình; nhận dạng và dịch chạy lần lượt, không đồng thời.',
             'Apple Silicon dùng tăng tốc Metal. Trên Windows, nhận dạng giọng nói chạy trên CPU (chậm hơn rõ rệt so với Mac cùng tầm), còn dịch chạy trên GPU có driver Vulkan, không có thì quay về CPU. Không hỗ trợ Mac Intel.',
...
T['th'] = dict(
  ...
  req_notes=['"แรมขณะใช้งาน" คือที่ใช้จริงหลังโหลดโมเดล การจับบทพูดกับการแปลทำทีละอย่าง ไม่กินแรมพร้อมกัน',
             'Apple Silicon ใช้การเร่งด้วย Metal ส่วนเวอร์ชัน Windows การรู้จำเสียงประมวลผลบน CPU (ช้ากว่า Mac ระดับเดียวกันอย่างเห็นได้ชัด) ขณะที่การแปลทำงานบน GPU ที่มีไดรเวอร์ Vulkan และกลับไปใช้ CPU หากไม่มี ไม่รองรับ Mac Intel',
...
def feedback_page(k, ok=False):
    f = FB[k]; p = '' if k == 'zh' else '../'
    home = p if p else './'
    css = CSS.replace('url(assets/hero-wave.jpg)', 'url(' + p + v('assets/hero-wave.jpg') + ')')
    head = (f'<!doctype html><html lang="{HTML_LANG[k]}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">\n'
            f'<title>{esc(f["title"])} · Wave Subs</title><meta name="robots" content="noindex"><link rel="icon" href="{p}{v("assets/icon.png")}">\n'
            f'<style>{css}\n'
            '  .fb{max-width:680px;margin:0 auto;padding:56px 24px 80px} .fb h1{font-size:clamp(26px,4vw,38px);margin:0 0 12px} .fb p.intro{color:var(--text-2);margin:0 0 28px;line-height:1.7}\n'
            '  .fb label{display:block;font-size:13.5px;color:var(--text-2);margin:18px 0 6px} .fb select,.fb input,.fb textarea{width:100%;box-sizing:border-box;background:var(--panel);border:1px solid var(--line);border-radius:10px;color:var(--text);padding:11px 12px;font:inherit;font-size:15px}\n'
            '  .fb textarea{min-height:180px;resize:vertical} .fb .btn{margin-top:22px;cursor:pointer} .fb .links{margin-top:28px;font-size:13.5px;color:var(--text-3)} .fb .links a{margin-right:16px}\n'
            '  .hidden{display:none}\n</style></head><body>\n'
            f'<header class="top"><div class="wrap"><a class="brand" href="{home}"><img src="{p}{v("assets/icon.png")}" alt="Wave Subs"><span>Wave Subs</span></a></div></header>\n'
            '<main class="fb">')
    foot = (f'<div class="links"><a href="{REPO}/discussions" target="_blank" rel="noopener">{esc(f["gh"])}</a>'
            f'<a href="{REPO}/issues" target="_blank" rel="noopener">{esc(f["issues"])}</a><a href="{home}">{esc(f["back"])}</a></div></main></body></html>\n')
    if ok:
    ...
    form = (f'<h1>{esc(f["h1"])}</h1><p class="intro">{esc(f["intro"])}</p>\n'
            ...
            f'  <label for="f-msg">{esc(f["msg"])}</label>\n'
            f'  <textarea name="message" id="f-msg" required placeholder="{esc(f["msg_ph"])}"></textarea>\n'
            f'  <label for="f-email">{esc(f["email"])}</label>\n'
            '  <input type="email" name="email" id="f-email" autocomplete="email">\n'
...
def build_all():
  ...
  for k in LANGS:
      ...
      open(os.path.join(fdir, 'feedback.html'), 'w', encoding='utf-8').write(feedback_page(k))
      open(os.path.join(fdir, 'feedback-ok.html'), 'w', encoding='utf-8').write(feedback_page(k, ok=True))
