#!/usr/bin/env python3
"""App Store Connect 多语言文案生成 + 字数校验。产出 store/app-store-localizations.md。
限制（按字符数）：名称 30、副标题 30、宣传文本 170、关键词 100、描述 4000、版本说明 4000。
关键词原则：逗号分隔不加空格，不重复名称/副标题里已有的词。"""
import os, sys

LIMITS = dict(name=30, subtitle=30, promo=170, keywords=100, description=4000, whatsnew=4000)

L = {}

L['zh-Hans'] = dict(asc='简体中文 (zh-Hans)', shots='zh-Hans',
 name='Wave Subs：AI 字幕生成与翻译',
 ...
 whatsnew='中国大陆也能下载模型了：连不上 huggingface.co 时自动切换镜像；下载认系统代理。修复部分 Windows 上中文系统显示英文的问题。',
...
L['en-US'] = dict(asc='English (U.S.) (en-US)', shots='en',
 name='Wave Subs: AI Subtitle Maker',
 ...
 whatsnew='Model downloads now work from mainland China (automatic mirror fallback, honours system proxy). Fixed some Windows setups showing English on a Chinese system.',
...
L['ja'] = dict(asc='日本語 (ja)', shots='ja',
 name='Wave Subs：AI 字幕生成・翻訳',
 ...
 whatsnew='モデルのダウンロードが中国本土でも可能に（ミラーへ自動切替、システムプロキシに対応）。一部の Windows で中国語環境なのに英語表示になる問題を修正。',
...
L['ko'] = dict(asc='한국어 (ko)', shots='ko',
 name='Wave Subs: AI 자막 생성·번역',
 ...
 whatsnew='중국 본토에서도 모델 다운로드 가능(미러 자동 전환, 시스템 프록시 지원). 일부 Windows에서 중국어 시스템인데 영어로 표시되던 문제 수정.',
...
L['fr-FR'] = dict(asc='Français (fr-FR)', shots='fr',
 name='Wave Subs : sous-titres par IA',
 ...
 whatsnew="Le téléchargement des modèles fonctionne désormais depuis la Chine continentale (bascule automatique vers un miroir, proxy système pris en charge). Correction de certains PC Windows affichant l'anglais sur un système en chinois.",
...
L['de-DE'] = dict(asc='Deutsch (de-DE)', shots='de',
 name='Wave Subs: KI-Untertitel',
 ...
 whatsnew='Modell-Downloads funktionieren jetzt auch aus Festlandchina (automatischer Spiegel-Fallback, Systemproxy wird beachtet). Behoben: Einige Windows-Systeme zeigten auf Chinesisch Englisch an.',
...
L['ru'] = dict(asc='Русский (ru)', shots='ru',
 name='Wave Subs: субтитры с ИИ',
 ...
 whatsnew='Загрузка моделей теперь работает из материкового Китая (автоматическое переключение на зеркало, учёт системного прокси). Исправлено: на некоторых Windows с китайской системой интерфейс был английским.',
...
L['id'] = dict(asc='Bahasa Indonesia (id)', shots='id',
 name='Wave Subs: Subtitle AI',
 ...
 whatsnew='Unduhan model kini berfungsi dari Tiongkok daratan (beralih otomatis ke cermin, mendukung proxy sistem). Memperbaiki beberapa Windows yang menampilkan bahasa Inggris pada sistem berbahasa Mandarin.',
...
L['ms'] = dict(asc='Bahasa Melayu (ms)', shots='ms',
 name='Wave Subs: Sari Kata AI',
 ...
 whatsnew='Muat turun model kini berfungsi dari tanah besar China (beralih automatik ke cermin, menyokong proksi sistem). Membetulkan sesetengah Windows yang memaparkan bahasa Inggeris pada sistem berbahasa Cina.',
...
L['vi'] = dict(asc='Tiếng Việt (vi)', shots='vi',
 name='Wave Subs: Phụ đề AI',
 ...
 whatsnew='Tải mô hình đã hoạt động từ Trung Quốc đại lục (tự chuyển sang máy chủ dự phòng, hỗ trợ proxy hệ thống). Sửa lỗi một số máy Windows hiển thị tiếng Anh trên hệ thống tiếng Trung.',
...
L['th'] = dict(asc='ไทย (th)', shots='th',
 name='Wave Subs: ซับไตเติล AI',
 ...
 whatsnew='ดาวน์โหลดโมเดลได้จากจีนแผ่นดินใหญ่แล้ว (สลับไปมิเรอร์อัตโนมัติ รองรับพร็อกซีระบบ) แก้ปัญหา Windows บางเครื่องแสดงภาษาอังกฤษบนระบบภาษาจีน',
...
for code, d in L.items():
    ...
    out += [f"## {d['asc']}", '', f"截图：`store/screenshots/{d['shots']}/`",
            ...
            f"**版本说明 1.0.3：** {d['whatsnew']}", '', '**描述：**', '```text', d['description'], '```', '']
