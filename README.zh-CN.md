...
**画面文字（macOS）：** 可选地用系统 Vision OCR 读出画面里的招牌、便签、短信、告示、标题，只保留剧情相关的，翻译后贴在原文旁边（ASS 定位，SRT 放顶部）。烧录字幕和片头片尾名单按几何规律识别并跳过。编辑器里语音字幕与画面文字分两个页签各自编辑。
...
2026-09 用 70 部整片（NAS 上的官方字幕轨与知名字幕组字幕作 ground truth，日语 36 部、英语 21 部、其余 13 部）跑出来的数字，供选模型时参考：

- **识别（whisper large-v3）**：英语 19 部平均 WER 17.4%（纪录片/访谈约 6%，官方剧集 10~13%）；日语 18 部逐字 CER 19.1%、读音 CER 13.8%——约三分之一的"错"只是写法差异（分かった / わかった）。德语、挪威语、意大利语的官方字幕多为压缩改写，不能按逐字比。
- **large-v3-turbo 只比 large-v3 差 0.8 个百分点**（12 部），却快约 3 倍——赶时间可以放心用。
- **翻译（日→中，本地 Qwen3）**：以人工字幕原文为输入时，语义准确率 8B 93.5%、14B 94.4%、32B 93.9%，三档几乎一样，默认 8B 即可；端到端（识别→翻译）约 83~85%，差距几乎全部来自识别错误。
- **时间轴**：逐帧掩码 F1 81.8%，字幕起点落在人工字幕 ±250ms 内的比例 57%；不同片源的人工字幕本身就有 100~300ms 的提前量差异。
...
| `check-signs` | 画面文字：分行/跟踪/名单时段/烧录字幕带的几何规则、判别对齐锚与念读兜底、同屏取舍、ASS/SRT 写出 |
